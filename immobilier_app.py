...
def main():
    ...
    if df is not None:
        ...
        with chart_tab1:
            # Department selector
            dept_list = ["Tous les départements"] + sorted(df['departement'].dropna().unique().tolist())
            selected_dept = st.selectbox(
                "Filtrer par département :",
                dept_list,
                key="dept_selector"
            )
            fig_price_type = create_price_by_type_chart(df, selected_dept)
        ...
        with chart_tab2:
            ...
            if selected_city == "Paris":
                paris_options = get_paris_arrondissements(df)
            ...
            fig_distribution = create_price_distribution_by_city(df, selected_city, selected_arrondissement)
